...
function editar(id) {
    var response = ajaxRequest("https://totaltravel.somee.com/API/HotelsActivities/Find?id=" + id);
...
function actualizar() {
    ...
    if (ValidateFormStatus) {
        ...
        var status = uploadFile(`https://totaltravel.somee.com/API/HotelsActivities/Update?id=${id}`, data, "PUT");
        if (status.code == 200) {
            location.reload();
