...
$(document).ready(async function () {
    ...
    $(document).ready(function () {
        var response = ajaxRequest(urlAPI + "/API/Users/Find?id=" + idUser);
...
function GetTypePartners() {
    ...
    var response = ajaxRequest(urlAPI +"/API/PartnerType/List");
    if (response.code == 200) {
...
function GetCountriesUpdate() {
    ...
    var response = ajaxRequest(urlAPI + "/API/Countries/List");
    if (response.code == 200) {
        for (var i = 0; i < response.data.length; i++) {
            var item = response.data[i];
...
async function GetImage() {
    var responseImage = ajaxRequest(urlAPI +"/API/RootFiles/GetAllImages?folderName=UsersProfilePics/User-" + idUser);
    ...
    if (userValidate) {
        ...
        if (Address_success) {
           ...
           const UserUpdateStatus = uploadFile(
               urlAPI + "/API/Users/Update?id=" + idUser, userUpdate, "PUT"
            );
